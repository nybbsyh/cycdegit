$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/devUser/getListInfo",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/Version/devMain?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/Version/toupdateVersions?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/Version/viewInfo/?id="+ obj.attr("appinfoid");
});
